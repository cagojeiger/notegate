...
- Recording feedback: The microphone signal is supplementary and `Recording` plus elapsed time remain the authoritative status. Reduced-motion users receive lower-frequency level changes without decorative pulsing.

## Responsive behavior

- Supported breakpoints/devices: Existing desktop/tablet/mobile layout policy remains authoritative.
- Layout adaptations: Login stays centered and bounded; docked Workbench sidebars are width-adjustable while mobile sidebar overlays keep their fixed responsive geometry; the Space Library uses one column on mobile, two on tablet, three on desktop, and four on wide desktop; the Space Inspector is right-docked on desktop/tablet and inline below the cards on mobile. Recording is one compact row above uploads on desktop/tablet and wraps its actions to a second row on narrow mobile widths without covering document content.
