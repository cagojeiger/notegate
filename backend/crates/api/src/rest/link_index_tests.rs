...
use notegate_db::{
    FilesRepo, LINK_IMPACT_JOB_KIND, LINK_SOURCE_JOB_KIND, LINK_SPACE_JOB_KIND, TextMutationKind,
    test_support::TestDb,
};
...
async fn drain_link_index(
    ...
    let kinds = vec![
        LINK_IMPACT_JOB_KIND.to_owned(),
        LINK_SOURCE_JOB_KIND.to_owned(),
        LINK_SPACE_JOB_KIND.to_owned(),
    ];
    for _ in 0..50 {
        let claims = queue
            .claim_many("rest-link-index-test", &kinds, Duration::from_secs(30), 16)
            .await?;
        if claims.is_empty() {
            return Ok(());
        }
        for claim in claims {
            ...
            let space_id = claim.payload["space_id"]
                .as_str()
                .ok_or("missing space_id")?
                .parse()?;
            match claim.kind.as_str() {
                LINK_IMPACT_JOB_KIND => {
                    let changed_node_id = claim.payload["changed_node_id"]
                        .as_str()
                        .ok_or("missing changed_node_id")?
                        .parse()?;
                    state
                        .link_index
                        .process_impact(&fence, space_id, changed_node_id)
                        .await?;
                }
                LINK_SPACE_JOB_KIND => {
                    state.link_index.process_space(&fence, space_id).await?;
                }
                LINK_SOURCE_JOB_KIND => {
                    let source_node_id = claim.payload["source_node_id"]
                        .as_str()
                        .ok_or("missing source_node_id")?
                        .parse()?;
                    state
                        .link_index
                        .process_source(&fence, space_id, source_node_id)
                        .await?;
                }
                _ => return Err("unexpected link job kind".into()),
            }
            if !queue.succeed(&claim).await? {
                return Err("link job claim was lost".into());
            }
        }
    }
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    sqlx::query(
        "DELETE FROM background_jobs \
         WHERE job_kind IN ('node_link_impact', 'node_link_source', 'node_link_space') \
