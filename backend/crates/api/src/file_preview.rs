use notegate_media::{
    DOCX_MEDIA_TYPE, UNKNOWN_MEDIA_TYPE, detect_media_type_from_bytes, is_docx_candidate,
    sniff_media_type,
};
...
pub async fn detect_object_media_type(
    ...
    let prefix = storage
        ...
        .await?;
    let inferred_media_type = sniff_media_type(&prefix).unwrap_or(UNKNOWN_MEDIA_TYPE);
    let media_type = if is_preview_size_allowed(byte_len)
        ...
        tokio::task::spawn_blocking(move || {
            let _permit = permit;
            let detection = detect_media_type_from_bytes(&bytes);
            if let Some(reason) = detection.docx_rejection {
