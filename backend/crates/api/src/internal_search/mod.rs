...
mod tests {
    ...
    #[tokio::test]
    async fn search_app_exposes_only_private_search_and_control_plane()
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let app = crate::routes::search_app(SearchServerState::new(
            ...
            10,
            runtime,
            crypto.internal_search_signing_key(),
            None,
        ));
    ...
    #[tokio::test]
    async fn private_search_listener_rejects_unsigned_calls_and_preserves_mcp_results()
    -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn internal_node_contract_serializes_exactly_like_existing_mcp_summary() {
        let now = chrono::Utc::now();
        let account_id = uuid::Uuid::new_v4();
        let view = notegate_model::files::NodeView {
            node: notegate_model::Node {
                id: uuid::Uuid::new_v4(),
                space_id: uuid::Uuid::new_v4(),
                parent_id: Some(uuid::Uuid::new_v4()),
                name: "note.md".to_owned(),
                kind: notegate_model::NodeKind::Text,
                sort_order: 3,
                metadata: serde_json::json!({"not": "returned"}),
                search_enabled: true,
                write_locked: false,
            ...
            has_children: false,
            text: Some(notegate_model::files::TextStats {
                content_sha256: "abc123".to_owned(),
                byte_len: 42,
                line_count: 2,
                storage_format: notegate_model::TextStorageFormat::Plain,
                at_rest_encryption: notegate_model::TextAtRestEncryption::Server,
            }),
            file: None,
            write_lock_sources: vec![notegate_model::files::WriteLockSource {
        ...
        let expected = crate::mcp::tools::resolve::node_summary(&view);
        let actual = serde_json::to_value(super::contract::SearchNodeSummary::from(view))
