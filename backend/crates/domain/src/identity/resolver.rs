...
pub trait UserStore: Clone + Send + Sync + 'static {
    fn upsert_by_sub(
        ...
        email: &str,
        name: &str,
    ) -> impl Future<Output = notegate_core::Result<User>> + Send;

    fn find_by_sub(
        &self,
        sub: &str,
    ) -> impl Future<Output = notegate_core::Result<Option<User>>> + Send;
}
...
mod tests {
    ...
    impl UserStore for MemoryUsers {
        ...
        async fn find_by_sub(&self, sub: &str) -> notegate_core::Result<Option<User>> {
            let guard = self
                .user
                .lock()
                .map_err(|error| Error::internal(format!("test lock failed: {error}")))?;
    ...
    #[tokio::test]
    async fn inactive_user_rejected() -> notegate_core::Result<()> {
    ...
    #[tokio::test]
    async fn browser_login_upserts_authenticated_user() -> notegate_core::Result<()> {
        let users = MemoryUsers::default();
    ...
    #[tokio::test]
    async fn registered_user_resolves_for_api() -> notegate_core::Result<()> {
        let users = MemoryUsers::default();
        {
            let mut guard = users
                .user
