...
async fn full_files_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let registration = single_upload_registration(upload_id, format!("objects/{upload_id}"));
    files
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    files.prepare_object_upload(owner, ws, &upload).await?;
    let registration = single_upload_registration(upload_id, "objects/logo.png");
    files
        .record_registered_object_upload(&registration, owner, ws, &upload)
        .await?;
    let uploaded = files
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, media_type) in [("first.bin", "image/png"), ("second.bin", "image/jpeg")] {
        ...
        files.prepare_object_upload(owner, ws, &upload).await?;
        let registration = single_upload_registration(upload_id, format!("objects/{name}"));
        files
            .record_registered_object_upload(&registration, owner, ws, &upload)
            .await?;
        let uploaded = files
