...
import { EventHistoryModal, type HistoryTab } from "../features/events/EventHistoryModal";
...
import { useUploadManager } from "../features/uploads/UploadProvider";
...
export function AppShell({ me, onSignOut }: AppShellProps) {
  ...
  const transferCount = uploads.activeCount + uploads.failedCount;
  const openHistory = (initialTab: HistoryTab = "files") => {
    closeMobilePanels();
    setHistoryScope({ initialSpaceId: workbench.activeSpace?.id ?? null, initialTab });
  };
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-bg text-text">
      ...
      <main className="relative flex min-h-0 flex-1 border-y border-seam">
        <ActivityRail spaces={workbench.spaces} activeSpace={workbench.activeSpace} canCreateSpace={workbench.canCreateSpace} canManageSpaces={workbench.canCreateSpace} transferCount={transferCount} onSelectSpace={actions.selectSpace} onReorderSpaces={actions.reorderSpaces} onCreateSpace={actions.promptCreateSpace} onRenameSpace={actions.promptRenameSpace} onDeleteSpace={actions.confirmDeleteSpace} onOpenHistory={() => openHistory(transferCount > 0 ? "transfers" : "files")} onOpenSettings={openSettings} />
      ...
      <MobileSpaceBar spaces={workbench.spaces} activeSpace={workbench.activeSpace} canCreateSpace={workbench.canCreateSpace} transferCount={transferCount} onSelectSpace={actions.selectSpace} onCreateSpace={actions.promptCreateSpace} onOpenHistory={() => openHistory(transferCount > 0 ? "transfers" : "files")} onOpenSettings={openSettings} />
      <StatusBar activeSpace={workbench.activeSpace} activeUploads={uploads.activeCount} failedUploads={uploads.failedCount} uploadProgress={uploads.progressPercent} onOpenTransfers={() => openHistory("transfers")} />
      <Toast />
      {historyScope ? <EventHistoryModal spaces={workbench.spaces} initialSpaceId={historyScope.initialSpaceId} initialTab={historyScope.initialTab} canViewAuditEvents={canViewAuditEvents(me)} onClose={() => setHistoryScope(null)} /> : null}
