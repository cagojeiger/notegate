...
export function MobileSpaceBar({ spaces, activeSpace, onSelectSpace, onCreateSpace, onSignOut }: { spaces: Space[]; activeSpace: Space | null; onSelectSpace: (space: Space) => void; onCreateSpace: () => void; onSignOut: () => void }) {
  return (
    <nav aria-label="Spaces" className="flex items-center gap-2 border-t border-border bg-surface px-3 py-2 md:hidden">
      ...
      <div className="ml-auto shrink-0 border-l border-border pl-2">
        <button type="button" aria-label="Settings" onClick={onSignOut} className="grid size-9 place-items-center rounded-full border border-border bg-panel text-muted">
          <Settings size={16} />
        </button>
      </div>
    </nav>
