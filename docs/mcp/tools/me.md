...
An MCP OAuth login proves the authgate identity, but notegate still requires a
local `users` row. The first local user is created only by the browser login
flow:

1. Open the configured login URL once, usually `${NOTEGATE_PUBLIC_URL}/login`.
...
If an authenticated MCP caller has no local notegate user row yet, `/mcp`
returns HTTP `403` with `error="not_registered"` and onboarding URLs:

```json
{
  "error": "not_registered",
  "message": "This authgate account is authenticated but not registered in notegate yet. Open login_url once, then reconnect your MCP client.",
  "login_url": "http://localhost:9191/login",
  "mcp_url": "http://localhost:9191/mcp"
}
```

The exact host is configuration-driven:

- `NOTEGATE_PUBLIC_URL`: browser-facing notegate origin used to build `login_url`.
...
For GitOps production these should normally be the public `notegate` domain; for
local development they may be `http://localhost:9191` and
`http://localhost:9191/mcp`.
...
- `id`: local notegate `users.id` UUID as a lowercase hyphenated string.
- `sub`: authgate subject.
- `email`: user email from the local notegate row.
