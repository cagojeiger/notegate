...
Quota에 포함되는 live usage의 의미, counter 갱신, 전체 재계산 정책은 `usage-and-quotas.md`를 따른다.
...
tier0
  ...
  space_max_content_bytes = 134217728    # 128 MiB live text+file content per space
...
system_max
  ...
  space_max_content_bytes = 1073741824   # 1 GiB
...
1만 user 기준 `system_max` hard-limit worst case는 다음과 같다. `tier0`만 사용하면 content worst case는 user당 128 MiB다.
...
tier0_logical_content_per_user = 128 MiB
tier0_logical_content_10000_users = 1.25 TiB
```
