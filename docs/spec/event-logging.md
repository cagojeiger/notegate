...
Retention은 policy decision이며 compliance readiness를 주장하기 전에 명시되어야 한다. Retention policy가 정해지기 전까지 event row는 identifier를 보존하되 embedded PII를 피하도록 설계한다. User anonymization 이후에도 attribution shell은 유지하되 개인 정보를 노출하지 않는 것이 목표다.
...
```text
security_events for failed auth/authorization attempts
admin/export APIs for compliance evidence
