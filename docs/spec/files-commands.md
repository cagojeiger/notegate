...
node를 삭제한다. 삭제는 즉시 일반 조회에서 숨겨지고, DB row는 내부 purge job이 처리할 때까지 soft-deleted 상태로 남는다.
...
- 삭제된 node는 public API/MCP에서 복구하지 않는다.
- 삭제된 node는 retention 기간 뒤 purge job으로 hard delete될 수 있다.
