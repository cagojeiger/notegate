...
- 잠긴 node 또는 잠긴 조상 아래에서는 create, Text write/append/patch/edit, metadata, 검색 정책, 저장 Text 암호화, rename, reorder, move, delete, File upload 등록/완료를 거부한다.
...
- 잠금 전에 시작한 미완료 File upload는 attach할 수 없다. Object 전송 뒤 attach가 잠금으로 거부되면 upload 원장을 즉시 `expire_pending`으로 전환하고 기존 cleanup worker가 object를 삭제한다. 정리 실패는 backoff로 재시도한다.
