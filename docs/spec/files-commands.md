...
- File upload 등록은 destination의 잠금을 검사하고 해당 upload handle의 write-lock 허가를 예약한다. 등록 뒤 destination이 잠겨도 기존 handle은 attach할 수 있으며, 잠금 이후 새 upload 등록은 거부한다. 완료 시 일반 write permission, parent 존재, 이름 충돌과 quota는 다시 확인한다. 취소되거나 만료된 upload object는 object storage cleanup reconciliation이 backoff로 정리한다.
