...
`find` result는 `schemas.md`의 `McpNodeSummary[] + Page`다. `grep` result는 `McpGrepSummary[] + Page`다. 본문 또는 metadata는 search 응답에 싣지 않는다. 자세한 내용은 `files_stat`, `files_read`로 조회한다.
