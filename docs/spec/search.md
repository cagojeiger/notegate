...
```text
find = nodes metadata/name search + derived path display
...
scope node를 찾는다. 이후 검색은 그 node의 subtree 안으로 제한한다.
...
3. scope node가 folder인지 확인한다.
4. scope folder 아래 subtree node id set을 recursive CTE로 계산한다.
5. find/grep query는 이 subtree id set 안에서만 실행한다.
6. 결과 path는 parent chain에서 derive한다.
```
...
```text
depth 0: scope folder
depth 1: scope folder의 children
...
Path substring search를 더 강하게 지원해야 하면 full-path string을 canonical column으로 되살리기보다
별도 search index/materialized view를 도입한다.
