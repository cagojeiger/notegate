...
Agent API는 agent account와 API key를 관리하는 user-only endpoint다. Agent의 workspace별 권한은 Access category에서 따로 부여한다. API key는 인증 시 `agent` account로 처리한다. Agent key lifecycle은 workspace role이 아니라 agent 생성자/소유자 규칙으로 관리한다. Workspace owner는 agent account에 viewer/editor workspace access를 grant/revoke할 수 있을 뿐, agent key를 관리하지 않는다.
