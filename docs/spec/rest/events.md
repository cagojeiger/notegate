...
```http
GET /api/v1/me/events?limit=50&cursor=...
```

User caller만 가능하다. Caller의 `owner_user_id` scope에 속한 `audit_events`를 `occurred_at desc, id desc` 순으로 반환한다.
...
```json
{
  "events": [
    {
      "id": 1042,
      "occurred_at": "2026-07-08T09:12:00Z",
