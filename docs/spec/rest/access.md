...
Access API는 workspace owner-only endpoint다. Owner는 `workspace_access.role='owner'`인 active user account다. Access row는 owner/editor/viewer membership을 저장하며, `workspaces.created_by`는 최초 생성자/audit attribution이다. Grant/revoke/downgrade side effect와 owner 보호 규칙은 `docs/spec/lifecycle.md`를 따른다. 한 workspace는 자동 owner row를 포함해 최대 `20`개의 active access row를 가지고, 한 account는 최대 `100`개의 live workspace에 접근할 수 있다.
...
대상 account가 active 상태일 때 `viewer`, `editor`, 또는 `owner` access를 생성하거나 role을 변경한다. 대상 account가 없거나 비활성/삭제 상태면 요청한 role과 무관하게 `404 not_found`로 거부한다(역할에 따라 코드가 달라지지 않는다). `owner` role은 active user account에만 허용하며, active agent account에 `owner`를 grant하면 `409 conflict`다(계정은 유효하나 role/kind가 충돌). 이미 revoke된 row가 있으면 같은 `(workspace_id, account_id)` row를 다시 활성화한다. 이때 현재 grant 상태의 `granted_by`/`granted_at`을 갱신한다. Workspace의 active access row가 `20`개를 넘거나 대상 account의 accessible live workspace가 `100`개를 넘으면 `409 conflict`로 거부한다. Creator owner row와 마지막 active user owner 보호는 `docs/spec/lifecycle.md`를 따른다.
