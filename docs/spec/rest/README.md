...
2. `/swagger-ui`를 연다.
3. 같은 origin browser session cookie로 `/api/v1/*`를 호출하거나, Swagger `Authorize`에 OAuth bearer JWT 또는 notegate API key credential을 넣어 호출한다. Cookie 기반 mutation 요청은 same-origin `Origin`/`Referer`가 필요하다.
