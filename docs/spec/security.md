...
- Token format version과 HMAC domain version은 별도 계약이다. 현재 token format은 v2이고 HMAC domain은 v1을 유지한다.
- 인증 시 token에서 계산한 `token_prefix`와 DB 값을 정확히 비교한다.
- 기존 `ngk_v1_` live key는 v2 전환 migration에서 revoke하며 다시 인증되지 않는다.
