...
pii_encryption_subkey      = HKDF(enc_root_secret, "notegate/enc/pii-encryption/v1")

lookup_epoch_verify_subkey = HKDF(lookup_root_secret, "notegate/lookup/epoch-verify/v1")
lookup_hmac_subkey         = HKDF(lookup_root_secret, "notegate/lookup/hmac/v1")
...
```text
verify_tag = HMAC(epoch_verify_subkey, "notegate/root-key-epoch:" + domain + ":" + key_id)
```
...
```text
display_name_ciphertext = AEAD_Encrypt(pii_encryption_subkey, display_name)
email_ciphertext        = AEAD_Encrypt(pii_encryption_subkey, email)
```
...
```text
provider_sub_hash = HMAC(lookup_hmac_subkey, provider + ":" + provider_subject)
email_hash        = HMAC(lookup_hmac_subkey, normalize(email))
```
...
token_hash     = HMAC(api_key_hmac_subkey, api_key_id + ":" + secret)
```
