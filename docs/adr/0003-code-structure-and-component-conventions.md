...
notegate 백엔드는 `core · model · db · service · api` 크레이트로 구성한다. 초기 구조에는 store
trait 기반 DIP, 도메인별 과분리, 표면별 파일 조직 불일치가 섞여 있어 탐색·유지 비용이 컸다.
...
- 단일 production 구현뿐인 store trait(`FilesStore`, `WorkspaceStore`, `AccountStore` 등)을 두지
...
- 진짜 두 번째 구현이 필요해지면(예: read-replica, 캐시, 다른 인증 백엔드) **그때 그 지점에 좁게**
  seam을 추가한다. 선제적으로 추상화를 깔지 않는다.
...
- 테스트 속도가 문제가 되면 mock이 아니라 **테스트 인프라**(마이그레이션-per-test 비용을 줄이는
  템플릿 스키마/재사용 등)로 해결한다.
...
- API 표면은 표면별 사용성에 맞춰 조직한다(REST는 리소스별, MCP는 `identity·workspaces·files·search`
...
- repo 자체 동작은 db 통합 테스트, files/search/permissions/lifecycle 같은 end-to-end 성격은
  service 통합 테스트로 둔다.
- 테스트 속도 최적화는 테스트 인프라 레벨에서 한다(추상화 도입이 아니다).
- 새로운 다형성 필요가 실제로 생기면 해당 지점에 국소 seam을 추가한다.
