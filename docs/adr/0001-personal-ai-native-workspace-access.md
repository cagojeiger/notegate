...
상세 구현 계약은 다음 spec 문서에 둔다.

```text
docs/spec/db.md         DB schema와 index
docs/spec/api.md        API category/layer/invariant
docs/spec/rest-api.md   HTTP REST endpoint 계약
docs/spec/mcp-tools.md  MCP tool 계약
docs/spec/search.md     검색 방식
docs/spec/performance-limits.md 성능/제한 정책
```
...
- REST는 UI 친화적으로 `workspace_id + node_id` 기반을 사용한다.
- MCP는 LLM/CLI 친화적으로 `workspace:/path` 표기와 workspace 이름 + path 기반 tool을 사용한다. 이를 위해 workspace/node 이름은 CLI-safe 문자로 제한한다. `workspace_id`는 모호성 해소용 fallback으로만 둔다.
- audit log/event history는 이번 단계에서 제외한다. 필요해지면 별도 ADR과 spec으로 추가한다.
- 파일/폴더 단위 공유는 보류한다.
