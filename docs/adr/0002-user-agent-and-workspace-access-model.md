...
workspace lifecycle owner는 `workspaces.created_by` user에서 derive한다. Owner 이전은 지원하지 않는다.
`workspace_access`에는 owner role을 저장하지 않고, workspace 내부 파일 접근용 `viewer`/`editor` grant만 저장한다.

```text
owner  = workspaces.created_by user에서 derive; workspace lifecycle 관리 + 파일 full access
viewer = read
editor = read + write
...
user만 가능하고, 생성자가 영구 lifecycle owner가 된다. Agent는 workspace에 별도로 연결되며,
viewer 또는 editor 역할만 받을 수 있다.
...
workspace lifecycle owner를 `workspace_access` row로 표현하지 않으면 owner grant, 마지막 owner 보호,
agent owner 금지 같은 별도 규칙이 필요 없다. 생성자 user가 영구 lifecycle owner라는 제품 정책과도 맞다.
...
- workspace lifecycle 작업은 `workspaces.created_by` user만 수행한다.
- `workspace_access`는 `viewer`/`editor` 파일 접근 grant만 저장한다.
- 파일/폴더 단위 공유는 보류한다.
