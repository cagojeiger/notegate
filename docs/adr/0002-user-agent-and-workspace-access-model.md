...
API key는 `api_keys.account_id`에 연결된 account의 kind로 행동 주체를 결정한다.

접근 제어 단위는 workspace로 한다. 파일, 폴더, node 단위 ACL은 제공하지 않는다.

workspace membership과 권한의 source of truth는 `workspace_access`다. `workspaces.created_by`는
최초 생성자/audit attribution으로 유지하며 권한 판정의 owner source로 쓰지 않는다.
...
API key를 user/agent 별도 테이블로 나누면 credential lifecycle, hash secret rotation, revoke/expiry 정책이 중복된다.
통합 `api_keys` 모델은 key 자체가 권한을 갖지 않고 연결된 account kind와 workspace access가 권한을 결정하게 하므로 더 단순하다.
사람 사용자 자동화가 필요한 테스트/CLI 사용성은 user API key로 해결하고, AI agent 자동화는 agent API key와 workspace grant로 제한한다.
...
- API key는 `api_keys.account_id`에 연결된 user 또는 agent credential로 취급한다.
