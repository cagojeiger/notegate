...
초기 제품은 팀 협업을 목표로 하지 않는다. Team/organization 확장은 필요해지면 별도 fork나 별도 제품 라인에서 다룬다.
...
Text   = UTF-8 content; read/write/patch/grep 가능
File   = binary/object content; stat/upload/download 가능, 직접 text read/patch/grep 불가
```
